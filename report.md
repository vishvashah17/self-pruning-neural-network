...
λ = 1.0 corresponds to low sparsity and highest accuracy.
λ = 5.0 provides the best trade-off between sparsity and accuracy.
λ = 20.0 leads to very high sparsity with a slight drop in accuracy.
...
There is a large spike near 0, representing pruned weights.
There is also a cluster away from 0, representing active weights.

Refer to: `gate_distribution.png`

---

## 5. Conclusion

The model effectively balances accuracy and sparsity, achieving up to ~89% sparsity while maintaining competitive performance. This confirms that L1 regularization on gate parameters enables dynamic self-pruning during training.
